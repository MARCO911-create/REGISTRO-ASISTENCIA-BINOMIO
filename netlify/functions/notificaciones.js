...
exports.handler = async () => {
  const d = new Date(); d.setUTCHours(d.getUTCHours() - 4); // Ajuste Hora Bolivia
  const h = d.getHours();
  let t = "Chala App 🔔", c = "¡No olvides marcar!";

  // Lógica de horarios para elegir el mensaje
  if (h < 10) c = alAzar(mensajes.mañana);
  else if (h < 12) { t = "Impulso Mañanero ✨"; c = alAzar(mensajes.motiva1); }
  else if (h < 14) c = alAzar(mensajes.almuerzo);
  else if (h < 15) c = alAzar(mensajes.tarde);
  else if (h < 16) { t = "Energía de Tarde 🔥"; c = alAzar(mensajes.motiva2); }
  else if (h < 18) { t = "Recta Final 🏁"; c = alAzar(mensajes.motiva3); }
  else if (h < 20) c = alAzar(mensajes.salida);
  else { t = "Reflexión del Día 🌙"; c = alAzar(mensajes.motiva4); }
  ...
  try {
    ...
    return { statusCode: 200, body: "Mensaje enviado: " + c };
